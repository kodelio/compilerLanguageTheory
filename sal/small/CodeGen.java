...
public class CodeGen {
	...
	static void writeStatementCode(Tree<Token> tree) {
        ...
        switch (token) {
			...
			case HALT : emit("return"); break;// generate code for a list of statements
            case STATEMENTLIST:
          ...
          case IF: {
			...
			case UNTIL: {
				...
				// the start of the loop - if the until condition isn't metcode returns to here
				setLabel(startLabel);
				// insert the code to be executed inside the loop
				writeStatementCode(tree.child(1)); 		// the point at which the test is made - if there is one
				setLabel(continueLabel);	// 'continue' goes to just before the test
////////////////////////////////////////////////////////////////////////
				//find out if atest is to be made
				Tree<Token> test = tree.child(0);
				if(test != null) {
					// there is an until part test for itwriteExpressionCode(test, INT_TYPE);  	// code for test
	...
	public static boolean writeExpressionCode(Tree<Token> tree) {
		...
		switch(token) {
            ...
            case LEN_STR:
                ...
                return INT_TYPE;//!!! Insert String operations here !!!
			}
